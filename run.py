...
import sys
import os
from nearestneighbors import getNearestNeighbor
...
TILE_BLOCK_SIZE = old_div(TILE_SIZE, max(min(TILE_MATCH_RES, TILE_SIZE), 1))
WORKER_COUNT = max(cpu_count() - 1, 1)
OUT_FILE = 'mosaic.jpeg'
EOQ_VALUE = None
OUT_DIR = 'main_img_vectors/'
...
class TileProcessor(object):
   ...
   def __process_tile(self, tile_path):
      try:
         ...
         w_crop = old_div((w - min_dimension), 2)
         h_crop = old_div((h - min_dimension), 2)
         img = img.crop((w_crop, h_crop, w - w_crop, h - h_crop))

         large_tile_img = img.resize((TILE_SIZE, TILE_SIZE), Image.ANTIALIAS)
         small_tile_img = img.resize((old_div(TILE_SIZE,TILE_BLOCK_SIZE), old_div(TILE_SIZE,TILE_BLOCK_SIZE)), Image.ANTIALIAS)
...
class TargetImage(object):
   ...
   def get_data(self):
      ...
      w_diff = old_div((w % TILE_SIZE),2)
      h_diff = old_div((h % TILE_SIZE),2)
      
      # if necesary, crop the image slightly so we use a whole number of tiles horizontally and vertically
      if w_diff or h_diff:
         large_img = large_img.crop((w_diff, h_diff, w - w_diff, h - h_diff))

      small_img = large_img.resize((old_div(w,TILE_BLOCK_SIZE), old_div(h,TILE_BLOCK_SIZE)), Image.ANTIALIAS)
...
def fit_tiles(work_queue, result_queue, tiles_data):
   ...
   while True:
      try:
         img_data, img_coords, filename = work_queue.get(True)
         if img_data == EOQ_VALUE:
            break
         tile_index = getNearestNeighbor(filename)
...
class MosaicImage(object):
   def __init__(self, original_img):
      ...
      self.x_tile_count = old_div(original_img.size[0], TILE_SIZE)
      self.y_tile_count = old_div(original_img.size[1], TILE_SIZE)
   ...
   try:
      ...
      for x in range(mosaic.x_tile_count):
         for y in range(mosaic.y_tile_count):
            filename = OUT_DIR + 'piece' + str(i).zfill(5) + '.jpg.npz'
            ...
            work_queue.put((list(original_img_small.crop(small_box).getdata()), large_box, filename))
            i += 1
            progress.update()
